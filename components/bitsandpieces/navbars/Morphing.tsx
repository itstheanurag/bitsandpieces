...
const MorphingNavbar = ({ logo, navItems, buttons, children }: NavbarProps) => {
  ...
  return (
    <nav className="w-full bg-linear-to-r from-emerald-500 via-teal-500 to-cyan-500 p-8">
      {children ? (
        <>{children}</>
      ) : (
        <motion.div
          ...
          className="max-w-6xl mx-auto bg-slate-900 rounded-xl shadow-2xl"
        >
              ...
              <div className="hidden md:flex items-center gap-8 relative">
                {itemsToRender.map((item, index) => (
                  <motion.a
                    ...
                    className="relative py-2 text-gray-300 font-medium cursor-pointer"
                    whileHover={{ scale: 1.05 }}
                  >
                    <span
                      className={activeIndex === index ? "text-emerald-400" : ""}
                    >
                      {item.label}
                    </span>
                    ...
                    {activeIndex === index && (
                      <motion.div
                        layoutId="morphing-underline"
                        className="absolute -bottom-1 left-0 right-0 h-0.5 bg-linear-to-r from-emerald-400 to-cyan-400 rounded-full shadow-lg shadow-emerald-500/50"
                        transition={{
                          type: "spring",
                          stiffness: 380,
                          damping: 30,
                ...
                {buttons || (
                  <div className="flex items-center gap-3 ml-4 pl-4 border-l border-slate-700">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="px-5 py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-semibold shadow-lg"
                    >
                      Launch App
                    </motion.button>
                  </div>
